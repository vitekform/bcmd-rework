        ...
        if (lang.equalsIgnoreCase("cs_cz")){
            if (sender instanceof Player){
                ...
                if (sender.hasPermission("bcmd.gamemode") || sender.hasPermission("bcmd.admin")){
                    if (args.length != 1){
                        ...
                        p.sendMessage(ChatColor.GREEN + "Nastavil jsi si gamemode na Spectator!");
                    }
                    else {
                        if (Bukkit.getOnlinePlayers().contains(Bukkit.getPlayer(args[0]))){
                            Player target = Bukkit.getPlayer(args[0]);
                            target.setGameMode(GameMode.SPECTATOR);
                            target.sendMessage(ChatColor.GREEN + p.getName() + " ti nastavil gamemode Spectator!");
                            p.sendMessage(ChatColor.GREEN + "Nastavil jsi gamemode Spectator hráči " + target.getName());
                        }
                        else {
                ...
                if (args.length != 1){
                    ...
                    if (Bukkit.getOnlinePlayers().contains(Bukkit.getPlayer(args[0]))){
                        Player target = Bukkit.getPlayer(args[0]);
                        target.setGameMode(GameMode.SPECTATOR);
                        target.sendMessage(ChatColor.GREEN + "Konzole ti nastavila gamemode Spectator!");
                        Bukkit.getLogger().log(Level.INFO, ChatColor.GREEN + "Nastavil jsi hráči " + target.getName() + " gamemode Spectator!");
                    }
                    else {
            ...
            if (sender instanceof Player){
                ...
                if (sender.hasPermission("bcmd.gamemode") || sender.hasPermission("bcmd.admin")){
                    if (args.length != 1){
                        p.setGameMode(GameMode.SPECTATOR);
                        p.sendMessage(ChatColor.GREEN + "You have set your gamemode to Spectator!");
                    }
                    else {
                        if (Bukkit.getOnlinePlayers().contains(Bukkit.getPlayer(args[0]))){
                            Player target = Bukkit.getPlayer(args[0]);
                            target.setGameMode(GameMode.SPECTATOR);
                            target.sendMessage(ChatColor.GREEN + p.getName() + " set your gamemode to Spectator!");
                            p.sendMessage(ChatColor.GREEN + "You have set gamemode Spectator to " + target.getName());
                        }
                        else {
                ...
                if (args.length != 1){
                    ...
                    if (Bukkit.getOnlinePlayers().contains(Bukkit.getPlayer(args[0]))){
                        Player target = Bukkit.getPlayer(args[0]);
                        target.setGameMode(GameMode.SPECTATOR);
                        target.sendMessage(ChatColor.GREEN + "Console set your gamemode to Spectator!");
                        Bukkit.getLogger().log(Level.INFO, ChatColor.GREEN + "You have set gamemode Spectator to " + target.getName() + "!");
                    }
                    else {
