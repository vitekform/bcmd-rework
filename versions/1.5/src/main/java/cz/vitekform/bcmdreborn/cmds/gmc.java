        ...
        if (lang.equalsIgnoreCase("cs_cz")){
            if (sender instanceof Player){
                ...
                if (sender.hasPermission("bcmd.gamemode") || sender.hasPermission("bcmd.admin")){
                    if (args.length != 1){
                        ...
                        p.sendMessage(ChatColor.GREEN + "Nastavil jsi si gamemode na Creative!");
                    }
                    else {
                        if (Bukkit.getOnlinePlayers().contains(Bukkit.getPlayer(args[0]))){
                            Player target = Bukkit.getPlayer(args[0]);
                            target.setGameMode(GameMode.CREATIVE);
                            target.sendMessage(ChatColor.GREEN + p.getName() + " ti nastavil gamemode Creative!");
                            p.sendMessage(ChatColor.GREEN + "Nastavil jsi gamemode Creative hráči " + target.getName());
                        }
                        else {
                ...
                if (args.length != 1){
                    ...
                    if (Bukkit.getOnlinePlayers().contains(Bukkit.getPlayer(args[0]))){
                        Player target = Bukkit.getPlayer(args[0]);
                        target.setGameMode(GameMode.CREATIVE);
                        target.sendMessage(ChatColor.GREEN + "Konzole ti nastavila gamemode Creative!");
                        Bukkit.getLogger().log(Level.INFO, ChatColor.GREEN + "Nastavil jsi hráči " + target.getName() + " gamemode Creative!");
                    }
                    else {
            ...
            if (sender instanceof Player){
                ...
                if (sender.hasPermission("bcmd.gamemode") || sender.hasPermission("bcmd.admin")){
                    if (args.length != 1){
                        p.setGameMode(GameMode.CREATIVE);
                        p.sendMessage(ChatColor.GREEN + "You have set your gamemode to Creative!");
                    }
                    else {
                        if (Bukkit.getOnlinePlayers().contains(Bukkit.getPlayer(args[0]))){
                            Player target = Bukkit.getPlayer(args[0]);
                            target.setGameMode(GameMode.CREATIVE);
                            target.sendMessage(ChatColor.GREEN + p.getName() + " set your gamemode to Creative!");
                            p.sendMessage(ChatColor.GREEN + "You have set gamemode Creative to " + target.getName());
                        }
                        else {
                ...
                if (args.length != 1){
                    ...
                    if (Bukkit.getOnlinePlayers().contains(Bukkit.getPlayer(args[0]))){
                        Player target = Bukkit.getPlayer(args[0]);
                        target.setGameMode(GameMode.CREATIVE);
                        target.sendMessage(ChatColor.GREEN + "Console set your gamemode to Creative!");
                        Bukkit.getLogger().log(Level.INFO, ChatColor.GREEN + "You have set gamemode Creative to " + target.getName() + "!");
                    }
                    else {
